<script lang="ts">
    ...
    $effect(() => {
        if (storeLobby.current?.turn_time_limit_ms !== undefined) {
        ...
        if (storeLobby.current?.custom_rules && Array.isArray(storeLobby.current.custom_rules)) {
            customRules = storeLobby.current.custom_rules;
        }
    });
